...
final class DbalProjectionCheckpointStoreUnitTest extends UnitTestCase
{
    public function test_that_load_defaults_to_the_start_of_history(): void
    {
        ...
        self::assertSame(0, (new DbalProjectionCheckpointStore($connection))->load('orders'));
    }
    ...
    public function test_that_load_returns_the_stored_position(): void
    {
        ...
        self::assertSame(42, (new DbalProjectionCheckpointStore($connection))->load('orders'));
    }

    public function test_that_save_rejects_a_negative_position_using_the_current_checkpoint(): void
    {
        $connection = $this->mock(Connection::class);
        $connection->shouldReceive('fetchOne')->once()->andReturn(6);
        $store = new DbalProjectionCheckpointStore($connection);

        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('Projection checkpoint orders cannot move backward from 6 to -1.');

        $store->save('orders', -1);
    }

    public function test_that_save_advances_an_existing_checkpoint_without_an_insert(): void
    {
        $connection = $this->mock(Connection::class);
        $connection->shouldReceive('executeStatement')->once()->andReturn(1);
        $connection->shouldNotReceive('insert');

        (new DbalProjectionCheckpointStore($connection))->save('orders', 7);
    }

    public function test_that_save_inserts_a_new_checkpoint_when_no_existing_checkpoint_advances(): void
    {
        $connection = $this->mock(Connection::class);
        $connection->shouldReceive('executeStatement')->once()->andReturn(0);
        $connection->shouldReceive('insert')->once()->with('projection_checkpoints', [
            'projector_name' => 'orders',
            'global_position' => 7,
        ]);
    ...
    public function test_that_save_rejects_an_insert_race_that_reveals_a_newer_checkpoint(): void
    {
        $connection = $this->mock(Connection::class);
        ...
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('Projection checkpoint orders cannot move backward from 8 to 7.');

        $store->save('orders', 7);
    }

    public function test_that_reset_updates_an_existing_checkpoint(): void
    {
        $connection = $this->mock(Connection::class);
        $connection->shouldReceive('update')->once()->andReturn(1);
        $connection->shouldNotReceive('insert');

        (new DbalProjectionCheckpointStore($connection))->reset('orders');
    }

    public function test_that_reset_inserts_a_missing_checkpoint(): void
    {
        $connection = $this->mock(Connection::class);
    ...
    public function test_that_reset_retries_a_concurrent_insert_as_an_update(): void
    {
        $connection = $this->mock(Connection::class);
        $connection->shouldReceive('update')->twice()->andReturn(0, 1);
        $connection->shouldReceive('insert')->once()->andThrow($this->uniqueConstraintViolation());
