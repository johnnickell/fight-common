...
final readonly class Uuid extends ValueObject implements Comparable
{
    ...
    private function __construct(
        ...
        private string $timeMid,
        private string $timeHiAndVersion,
        private string $clockSeqHiAndReserved,
        private string $clockSeqLow,
        private string $node
    ) {
    }
     ...
     * @throws DomainException When the namespace is not a valid UUID
     */
    private static function uuid3(Uuid|string $namespace, string $name): static
    {
     ...
     * @throws DomainException When the namespace is not a valid UUID
     */
    private static function uuid5(Uuid|string $namespace, string $name): static
    {
        if (!($namespace instanceof self)) {
            $namespace = static::parse($namespace);
        }
     ...
     */
    private static function fromUnformatted(string $hex, int $version): static
    {
     ...
     * @throws DomainException When timestamp is not 8 hex octets
     */
    private static function guardTimestamp(string $timestamp): void
    {
     ...
     * @throws DomainException When clockSeq is not 14-bit unsigned
     */
    private static function guardClockSeq(int $clockSeq): void
    {
     ...
     * @throws DomainException When the node is not 6 hex octets
     */
    private static function guardNode(string $node): void
    {
