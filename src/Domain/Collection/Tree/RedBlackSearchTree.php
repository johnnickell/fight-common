...
use Fight\Common\Domain\Collection\Contract\ItemList;
...
final class RedBlackSearchTree implements BinarySearchTree
{
     ...
     */
    private function nodeRemove(mixed $key, RedBlackNode $node): ?RedBlackNode
    {
        $comp = $this->comparator->compare($key, $node->key());
        if ($comp < 0) {
     ...
     */
    private function nodeRemoveLeft(mixed $key, RedBlackNode $node): RedBlackNode
    {
        if (
     ...
     * @internal Helper for nodeRemove()
     */
    private function nodeRemoveRight(mixed $key, RedBlackNode $node): RedBlackNode
    {
        if (
     ...
     */
    private function nodeRemoveMax(RedBlackNode $node): ?RedBlackNode
    {
        if ($this->isRed($node->left())) {
            $node = $this->rotateRight($node);
        }
     ...
     */
    private function nodeSelect(int $rank, RedBlackNode $node): RedBlackNode
    {
     ...
     */
    private function rotateLeft(RedBlackNode $node): RedBlackNode
    {
        $link = $node->right();
     ...
     */
    private function rotateRight(RedBlackNode $node): RedBlackNode
    {
        $link = $node->left();
     ...
     */
    private function flipColors(RedBlackNode $node): void
    {
     ...
     */
    private function moveRedLeft(RedBlackNode $node): RedBlackNode
    {
     ...
     */
    private function moveRedRight(RedBlackNode $node): RedBlackNode
    {
        $this->flipColors($node);
     ...
     */
    private function balanceOnRemove(RedBlackNode $node): RedBlackNode
    {
     ...
     */
    private function balanceOnInsert(RedBlackNode $node): RedBlackNode
    {
        if (
            $this->isRed($node->right())
            && !$this->isRed($node->left())
