<?php

declare(strict_types=1);

namespace Fight\Common\Adapter\DependencyInjection;

use Fight\Common\Adapter\ServiceContainer\Symfony\CommandFilterCompilerPass as CanonicalCommandFilterCompilerPass;

/**
 * Class CommandFilterCompilerPass
 *
 * @deprecated since 1.2.0, use Fight\Common\Adapter\ServiceContainer\Symfony\CommandFilterCompilerPass
 */
final class CommandFilterCompilerPass extends CanonicalCommandFilterCompilerPass
{
}
