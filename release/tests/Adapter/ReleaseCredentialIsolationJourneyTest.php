...
final class ReleaseCredentialIsolationJourneyTest extends UnitTestCase
{
    ...
    public function test_that_release_journeys_and_provider_fakes_never_consult_or_expose_production_credentials(): void
    {
        ...
        foreach (
            [
                ['inspect', '--fixture='.$root.'/release/fixtures/inspect-candidate.json'],
                [
                    'plan',
    ...
    private function observeCanonicalChildEnvironment(string $root, array $environment): array
    {
        ...
        for ($attempt = 0; $attempt < 200; $attempt++) {
            ...
            foreach ($candidates as $candidateProcessId) {
                ...
                if (
                    ...
                    || !str_contains($candidateCommandLine, '/release/scripts/release.php')
                ) {
                    continue;
                }
        ...
        self::assertStringContainsString('/release/scripts/release.php', $commandLine);
        $entries = explode(
            "\0",
        ...
        file_put_contents(
            $fifo,
            file_get_contents($root.'/release/fixtures/inspect-candidate.json')
        );
        $process->wait();
