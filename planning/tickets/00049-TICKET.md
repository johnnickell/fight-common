...
- [x] The base `Arrayable` contract promises `array<array-key, mixed>` and each existing implementation retains
      or declares its honest narrower list or map shape.
- [x] `ResultSet<TRecord>` remains valid for arbitrary existing record types and preserves its current
      pagination fields and general-purpose `toArray` behavior.
- [x] Immutable `JSendEnvelope` and `JSendStatus` types represent success, fail, and error without owning an HTTP
      status, headers, framework response, service lookup, or I/O.
- [x] A single `Arrayable` presentation value becomes JSend data without pagination metadata.
- [x] A `ResultSet` of `Arrayable` records projects each record through `toArray` and preserves page, per-page,
      total-page, total-record, and records fields.
- [x] A non-`Arrayable` record fails only at the typed JSend presentation boundary and remains valid for every
      other `ResultSet` consumer.
- [x] The envelope owns final JSON encoding with option `79` and throwing error behavior; invalid UTF-8 and
      unencodable values raise `JsonException` before native response construction.
- [x] The canonical Symfony response accepts an envelope plus controller-selected status and headers, exposes
      ergonomic semantic factories, sets the native JSON content type, and uses the encoded body without
      re-encoding it.
- [x] The legacy Symfony response retains raw arrays, its option-79 default, caller-selected encoding options,
      and existing public behavior throughout `1.x`, with PHPDoc/documentation deprecation but no runtime notice.
- [x] Consumer probes cover exact success, fail, and error fields; null and optional data; single and paginated
