...
- [x] Exception and validation subscribers are published under the canonical Symfony HTTP namespace.
- [x] JSON request middleware is published under `Adapter\Middleware\Symfony`, and the error controller is
      published under `Adapter\Http\Symfony\Controller`.
- [x] The Symfony filesystem and URL generator are published under their capability-first provider namespaces.
- [x] Every superseded public FQCN remains independently loadable, functional, and documented as deprecated
      throughout `1.x` without emitting a runtime notice.
- [x] Compatibility mechanisms are selected per declaration from construction, extension, reflection, attribute,
      serialization, and registration evidence rather than applying aliases indiscriminately.
- [x] Real Symfony registration probes cover subscriber attributes and tags, controller resolution, middleware
      invocation, service identity, and old/new interoperability.
- [x] HTTP probes preserve validation and exception translation, native response status, headers, encoded body,
      and the typed JSend boundary delivered by T-00049.
- [x] Filesystem and URL-generation probes preserve success, failure translation, named-route parameters,
      absolute and relative output, and both public identities.
- [x] No legacy declaration is removed before `2.0.0`, and no framework dependency enters an inward layer.
