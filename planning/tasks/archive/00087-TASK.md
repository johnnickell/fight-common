...
Create one repository-local implementation task in each Symfony, Laravel, Yii, CodeIgniter, and Slim starter
for the lean, complete pre-submit gate required by ADR 0026 and TICKET-00018. Fight Common retains the portfolio
contract; each starter owns its framework-specific tooling, implementation, verification, and delivery state.

## Scope

- In scope: five local task handoffs, Fight Common 1.2 and PHPCS-standard adoption, framework-specific
...
- [x] Each successor identifies its repository-specific certification cleanup inventory while preserving completed historical tasks as history.
...
- `./bin/planning-check` passes in Fight Common after all five repository-local task links are recorded.
- Each linked starter task names its focused checks, canonical `./bin/build`, exact coverage evidence, and hosted
  CI state separately.

## Completion Notes

Fight Common `v1.2.0` was independently qualified through TASK-00041. The five successor tasks above are the authoritative implementation frontier; no starter gate, local `./bin/build`, or hosted-CI result is claimed here.
