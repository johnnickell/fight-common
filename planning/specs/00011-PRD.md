...
T-00017 has accepted the `d262866` certification and tree-identical `ef64fcb` merge. Under ADR 0027, T-00035 is
the thin, human-operated outcome that freshly certifies the exact remote `main` merge before signing and creating
a verified draft with the certified Composer tar, `certification.json`, and `SHA256SUMS` for immutable publication. T-00041 remains the
separate Packagist observation and published-package installation outcome. Each external effect requires explicit
authority and independent postcondition verification.
