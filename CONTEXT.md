...
| **MCP capability** | An explicit consumer-registered handler that owns a bounded set of MCP methods, capability advertisement, outer-parameter validation, and semantic result construction. Standard methods must advertise their matching standard capability family, and an advertised standard family requires its mandatory anchor method; discovery definitions and server identity contain JSON-safe Unicode values at composition. Subscription-driven notification flags remain false in this foundation until a later subscription contract can prove delivery behavior. Custom extensions remain explicit. The capability registry does not automatically expose ordinary Actions or CQRS handlers. |
| **MCP request metadata** | Bounded protocol context supplied under `_meta`: protocol version, client capabilities/information, and progress token. Metadata keys, defined fields, present trace context, and schema-permitted implementation icon sources are validated as protocol inputs, including RFC 2397 Base64 image data whose raw media components use RFC 2396 URL characters or valid percent escapes, whose decoded subtype and parameter attributes are RFC 2045 MIME tokens, and whose parameter values may use correctly escaped tspecial-bearing or quoted-string forms. Schema-permitted empty strings and open objects are retained. It is neither a credential nor a principal and does not convey authorization policy. |
...
| **Candidate commit** | The exact clean `HEAD` commit certified by `bin/release certify`. A branch name or dirty checkout cannot substitute for this immutable identity. |
| **Certification record** | The compact, digest-bearing JSON record emitted by `bin/release certify`. It binds the version, candidate commit, archive digest, dependency lanes, package surface, installed-consumer result, and cited historical starter receipts. |
| **Publication authorization** | Human approval for one bounded merge, tag, push, GitHub, Packagist, or deployment effect against the exact certified candidate. Certification grants none of these effects. |
| **Partial publication** | A fail-closed condition in which an external effect may have occurred but its postcondition is uncertain. Recovery requires operator-directed reconciliation and fresh verification. |
...
| Term | Meaning in Fight Common |
| --- | --- |
| **Signed release tag** | A separately authorized annotated tag whose signature, name, and peeled commit are verified against the accepted certification record. Certification does not create it. |
...
| **Incomplete publication** | A fail-closed condition entered when Packagist propagation, metadata, installation, or external-effect verification cannot complete. Recovery is explicit and remains bound to the certified candidate and publication receipt. |
...
| **Release archive** | The Composer archive named `fight-common-X.Y.Z.tar` produced from the exact clean candidate commit. Its SHA-256 is recorded in the certification record. |
| **Clean-install receipt** | Evidence from a temporary Composer consumer that installs the candidate archive with `--no-dev`, exercises representative public behavior, and proves the maintainer-only `Fight\Release\` namespace is unavailable. |
| **Release command** | The repository-owned `bin/release certify <version>` executable. It certifies a clean exact commit and performs no merge, tag, push, GitHub, Packagist, or deployment effect. |
