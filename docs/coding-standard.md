...
annotations), checks declaration docblock alignment and ignores source suppression annotations. Maintainers repair violations rather than adding
...
image available to verify compliant source, rejected inline/misaligned docblocks, ineffective suppression
annotations and exact, repeatable formatter output against real PHPCS/PHPCBF execution. This explicit maintainer qualification stays outside the default product build.
